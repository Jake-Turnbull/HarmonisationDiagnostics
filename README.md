...
`CrossSectionalReport()` is the single-dataset workflow.
`CrossSectionalComparisonReport()` compares multiple harmonised outputs side by side and generates a category-based scorecard across additive, multiplicative, linear-modelling, distributional, and PCA diagnostics.
...
**CrossSectionalComparisonReport():**
Compares multiple harmonised datasets against the same batch and covariate structure.
